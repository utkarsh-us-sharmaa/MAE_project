function dT_SS = SS_Heating(f,lambda,C,t,eta,r_pump,r_probe,absorbance,A_tot_powermeter)
...
if nargin==0 % only used if not called as an external function
    ...
    t=[100 1 500 1e6]*1e-9; %m ("Al" thickness estimated at 84nm=81nm Al+3nm oxide, from picosecond acoustic)
...
dT_SS  =rombint_multi(@(kvect) TDTR_TEMP_V4(kvect,f,lambda,C,t,eta,r_pump,r_probe,A_abs),kmin,kmax,1);
%-----------------------------------------------------------------